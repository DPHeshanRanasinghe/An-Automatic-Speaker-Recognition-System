MFCC FROM SCRATCH - COMPLETE MATLAB IMPLEMENTATION (39-DIMENSIONAL FEATURES)
============================================================================

This package contains a complete, from-scratch MFCC implementation in MATLAB
that extracts 39-dimensional features for speaker recognition:
...
7. apply_dct.m             - Step 8: DCT (get 13 MFCCs)
8. compute_delta.m         - Compute delta and delta-delta features
9. mfcc.m                  - MAIN function (outputs 39 features)
10. melfb.m                - Mel filterbank generator
...
% Extract 39-dimensional features with progress display
[features, mfcc, delta, delta2] = mfcc(audio, fs, true);

% features is (num_frames × 39)
%   columns 1-13:  MFCC coefficients
%   columns 14-26: Delta coefficients
%   columns 27-39: Delta-Delta coefficients

% Silent mode (no progress output)
features = mfcc(audio, fs, false);

COMPLETE PIPELINE WITH PROGRESS OUTPUT
======================================
When you run: [features, mfcc, delta, delta2] = mfcc(audio, fs, true);
...
========================================
Signal: 16000 samples (1.00 sec @ 16000 Hz)

[Step 1/8] Pre-emphasis filter...
  ✓ Applied pre-emphasis (α=0.97)

[Step 2/8] Frame blocking...
  ✓ Created 129 frames (256 samples, 100 hop, 60.9% overlap)

[Step 3/8] Applying Hamming window...
  ✓ Windowed 129 frames

[Step 4/8] Computing FFT and power spectrum...
  ✓ Power spectrum: 129 frames × 257 bins (31.2 Hz resolution)

[Step 5/8] Applying mel filterbank...
  ✓ Mel energies: 129 frames × 26 filters

[Step 6/8] Applying logarithm...
  ✓ Log-mel energies computed

[Step 7/8] Applying DCT...
  ✓ MFCC coefficients: 129 frames × 13 coeffs

[Step 8/8] Computing delta features...
  ✓ Delta coefficients: 129 frames × 13 coeffs
  ✓ Delta-delta coefficients: 129 frames × 13 coeffs

========================================
  EXTRACTION COMPLETE!
========================================
Final output: 129 frames × 39 features
  • MFCC:        columns 1-13
  • Delta:       columns 14-26
  • Delta-Delta: columns 27-39
...
========================
features     - Complete 39-dimensional feature matrix (num_frames × 39)
mfcc         - MFCC coefficients only (num_frames × 13)
delta        - Delta coefficients (num_frames × 13)
delta2       - Delta-delta coefficients (num_frames × 13)

For speaker recognition, use the full 39-dimensional 'features' matrix.
...
text(5, 7, 'MFCC', 'Color', 'white', 'FontWeight', 'bold');
text(5, 20, 'Delta', 'Color', 'white', 'FontWeight', 'bold');
text(5, 33, 'Delta-Delta', 'Color', 'white', 'FontWeight', 'bold');
hold off;

% Plot first 3 coefficients from each type
figure;
subplot(3,1,1);
plot(mfcc(:,1:3), 'LineWidth', 1.5);
title('MFCC Coefficients 0-2');
legend('c0', 'c1', 'c2');
grid on;

subplot(3,1,2);
plot(delta(:,1:3), 'LineWidth', 1.5);
title('Delta Coefficients 0-2');
legend('Δc0', 'Δc1', 'Δc2');
grid on;

subplot(3,1,3);
plot(delta2(:,1:3), 'LineWidth', 1.5);
title('Delta-Delta Coefficients 0-2');
legend('ΔΔc0', 'ΔΔc1', 'ΔΔc2');
grid on;
