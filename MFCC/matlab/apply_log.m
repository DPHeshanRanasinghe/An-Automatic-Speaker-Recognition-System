...
% APPLY_LOG - Apply logarithm to mel filterbank energies
%
% This is Step 7 of the MFCC pipeline. The logarithm compresses the
% dynamic range and matches human perception of loudness (Weber-Fechner law).
%
% INPUT:
%   mel_energies - mel filterbank energies (num_frames × num_filters)
%
% OUTPUT:
%   log_mel - logarithm of energies (same size as input)
%
% MATH:
%   log_mel[i,j] = log(mel_energies[i,j] + ε)
%   where ε = small constant to prevent log(0) = -Inf
%
% USAGE:
...
%   log_mel = apply_log(mel_energies);
%   
%   % Compare before/after log
%   figure;
%   subplot(2,1,1);
%   imagesc(mel_energies'); axis xy; colorbar; title('Linear Mel Energies');
%   subplot(2,1,2);
%   imagesc(log_mel'); axis xy; colorbar; title('Log Mel Energies');
%   % Notice the compressed dynamic range!

% =========================================================================
% Set epsilon value
% =========================================================================
% We add a small constant to avoid log(0) = -Inf
% Common choices:
%   - eps: MATLAB's machine epsilon (≈ 2.22e-16, smallest positive number)
%   - 1e-10: Small but larger than eps, avoids underflow issues
%   - 1e-8: Even safer for noisy signals

epsilon = eps;  % Use MATLAB's eps (safest, smallest)
% epsilon = 1e-10;  % Alternative: slightly larger safety margin

% =========================================================================
% Apply logarithm
% =========================================================================
% Natural logarithm (base e)
% Note: log() in MATLAB is natural log, log10() is base-10
log_mel = log(mel_energies + epsilon);

% Alternative with explicit floor:
% floor_value = 1e-10;
